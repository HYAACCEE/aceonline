...
-- Created by Administrator on 2017/2/6, 15:37:07 --
----------------------------------------------------

set define off
spool PCACE_20170206_02.log
...
from pcace.S_User a,pcace.S_Role b,pcace.Y_Emp c,pcace.Y_Organ d,pcace.Y_Department e,pcace.Y_JobPosition f
...
begin
  Result := '';
  DECLARE CURSOR v_rs IS select c5||'|'||c9 from B_Candidate_CVWork where c1=cid and c2=130001 order by c3 desc;
  begin
    open v_rs;
    LOOP
...
create or replace package body pcace.pkg_pcace_web is
  ...
  begin
    if a_type = 'add' or a_type = 'edit' then
      ...
      select c1,c5 into a_userName,a_loginName from Y_Emp where dataid = a_id;
      ...
      update Y_Emp set c16='已启用' where dataid=a_id;

    elsif a_type = 'stop' then

      select c1,c5 into a_userName,a_loginName from Y_Emp where dataid = a_id;
      Message := '停用<'||a_userName||'>';
      update Y_Emp set c16='<font color=red>已停用</font>' where dataid=a_id;
  ...
  begin
    ...
    update B_Candidate set
           ...
           c11 = v_age,
           c12 = v_xingzuo,
           (c41,c38,c40,c42,c43,c44,c45)= (select c5,c7,decode(c4,'至今',1,0),c9,c11,c12,c13 from (select rownum rownum_,m.* from B_Candidate_CVWork m where c1=a_cid order by c3 desc) where rownum_=1)
     where c1=a_cid;
  ...
  begin
    ...
    --入库审核规则:R00001
    DECLARE CURSOR v_rs IS select b.c1,b.c2,b.c4,b.c5,b.c8 from Y_APPModule a, Y_APPModuleDetail b where a.dataid=b.parentDataid and a.c6=130001 and b.c9=130001 and a.c1='R00001' order by b.dataid;
    BEGIN
      open v_rs;
      LOOP
  ...
  begin
    v_result := '';
    --顾问面评审核规则:R00002
    DECLARE CURSOR v_rs IS select b.c1,b.c2,b.c4,b.c5 from Y_APPModule a, Y_APPModuleDetail b where a.dataid=b.parentDataid and a.c6=130001 and b.c9=130001 and a.c1='R00002' order by b.dataid;
    BEGIN
      open v_rs;
      LOOP
        FETCH v_rs INTO v_title, v_type, v_tableName, v_content;
        exit when v_rs%notfound;

        execute immediate 'select count(dataid) from '||v_tableName||' where c1='||a_cid into counter;
        if counter=0 then
          if v_type = 11 then
            v_result := v_result || v_title || '不能为空！<br>';
          end if;
        else
          execute immediate 'select '||v_content||' from '||v_tableName||' where c1='||a_cid into v_content;
          if length(v_content) >0 then
            v_result := v_result || v_content;
          end if;
        end if;
      END LOOP;
      close v_rs;
    ...
    update B_Candidate set c89=v_result where c1=a_cid;
    commit;
