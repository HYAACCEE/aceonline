...
-- Created by Administrator on 2016/12/11, 7:31:29 --
-----------------------------------------------------

set define off
spool PCACE_20161211_02.log
...
create or replace package body pcace.pkg_pcace_web is
  ...
  begin
    if a_type = 'add' or a_type = 'edit' then
      ...
      for N in 1..a_preCount loop
          ...
          elsif N=3 then a_typeLevel := to_number(tmp);
      ...
      if a_type = 'add' then
        ...
        insert into I_DataParamType(paramClassID,className,classNameEn,typeLevel,parentID,childNum,memo,state,Istdate,Uptdate)
             values(a_paramClassID,a_className,a_classNameEn,a_typeLevel,a_parentID,0,a_memo,a_state,sysdate,sysdate);
  ...
  begin
    ...
    if a_type = 'add' and a_dataid = -1 then Message := Message || 'dataid=' || v_tmpdataid; end if;
    commit;
  exception when others then
    rollback;
