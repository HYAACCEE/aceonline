...
-- Created by Administrator on 2017/1/23 星期一, 09:03:03 --
---------------------------------------------------------

set define off
spool PCACE_20170123P_02.log
...
create or replace function pcace.F_Candidate_WorkYear(cid in varchar2) return varchar2 is
  Result varchar2(200);
begin
  select sum(to_number(substr(c4,1,4)) - to_number(substr(NVL(c3,to_char(sysdate,'yyyy')),1,4))) into Result from B_Candidate_CVWork where c1=cid;
  return(Result);
