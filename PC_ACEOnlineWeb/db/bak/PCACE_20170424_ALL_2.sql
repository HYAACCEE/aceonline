...
-- Created by Administrator on 2017/4/24, 3:06:52 --
----------------------------------------------------

set define off
spool PCACE_20170424_ALL_2.log
...
(
  ...
  c28          VARCHAR2(200),
  c29          VARCHAR2(500),
  c30          VARCHAR2(2000) default '0',
  c31          CHAR(1) default '0',
  c32          CHAR(1),
  c33          CHAR(1) default '0',
  c34          VARCHAR2(30),
  c35          VARCHAR2(30),
  c36          VARCHAR2(30),
  c37          VARCHAR2(30),
...
prompt
prompt Creating table A_SCHOOL
prompt =======================
prompt
create table PCACE.A_SCHOOL
(
  dataid       NUMBER(11) not null,
  datasctlevel NUMBER(6) default -1,
  datastatus   NUMBER(6) default -1,
...
prompt
prompt Creating table B_STAT_HC
prompt ========================
prompt
create table PCACE.B_STAT_HC
(
  dataid       NUMBER(11) not null,
  datasctlevel NUMBER(6) default -1,
  datastatus   NUMBER(6) default -1,
...
prompt
prompt Creating table B_T_CANDIDATE_INVITELOG
prompt ======================================
prompt
...
alter table PCACE.B_T_CANDIDATE_INVITELOG
...
prompt
prompt Creating table B_T_CANDIDATE_VIEWLOG
prompt ====================================
prompt
...
prompt
prompt Creating table B_T_USERQUESTION
prompt ===============================
prompt
create table PCACE.B_T_USERQUESTION
(
  dataid       NUMBER(11) not null,
  datasctlevel NUMBER(6) default -1,
  datastatus   NUMBER(6) default -1,
...
alter table PCACE.B_T_USERQUESTION
  add constraint B_T_USERQUESTION_PK primary key (DATAID)
  using index 
  tablespace TABLESPACE_PCACE
  pctfree 10
  initrans 2
...
prompt
prompt Creating table ZA_SCHOOL
prompt ========================
prompt
create table PCACE.ZA_SCHOOL
(
  iid          NUMBER(14) not null,
  imemo        VARCHAR2(500),
  dataid       NUMBER(11),
...
prompt
prompt Creating table ZB_STAT_HC
prompt =========================
prompt
create table PCACE.ZB_STAT_HC
(
  iid          NUMBER(14) not null,
  imemo        VARCHAR2(500),
  dataid       NUMBER(11),
...
prompt
prompt Creating table ZB_T_CANDIDATE_INVITED
prompt =====================================
prompt
create table PCACE.ZB_T_CANDIDATE_INVITED
(
  iid          NUMBER(14) not null,
  imemo        VARCHAR2(500),
  dataid       NUMBER(11),
...
prompt
prompt Creating table ZB_T_CANDIDATE_INVITELOG
prompt =======================================
prompt
...
prompt
prompt Creating table ZB_T_CANDIDATE_VIEWLOG
prompt =====================================
prompt
...
prompt
prompt Creating table ZB_T_USERQUESTION
prompt ================================
prompt
create table PCACE.ZB_T_USERQUESTION
(
  iid          NUMBER(14) not null,
  imemo        VARCHAR2(500),
  dataid       NUMBER(11),
...
prompt
prompt Creating sequence SEQB_T_USERQUESTION
prompt =====================================
prompt
create sequence PCACE.SEQB_T_USERQUESTION
minvalue 1
maxvalue 99999999999999
start with 1
increment by 1
...
prompt
prompt Creating function F_ISCONTAIN
prompt =============================
prompt
create or replace function pcace.F_isContain(
  src            in varchar2,
...
create or replace package body pcace.pkg_pcace_orderStat is
  ...
  begin
    ...
    if 1=1 or a_curdate = 2344 then
      ...
      --今日新增
      plog := 111;
      delete from B_Stat_HC where c1=260001;
       ...
       select seqB_Stat_HC.nextval, 260001, 130001, ttt.*
         from (select stat_c3, count(distinct stat_c4), count(distinct stat_c5), count(distinct stat_c6)
               ...
               from (select p.c2 stat_c3, --顾问
                            ...
                            decode(trunc(p.c81), trunc(sysdate), decode(p.c3, 240020, p.c1, ''), '') stat_c5, --入库中
       ...
       select seqB_Stat_HC.nextval, 260002, 130001, ttt.*
         from (select stat_c3, count(distinct stat_c4), count(distinct stat_c5), count(distinct stat_c6)
               ...
               from (select p.c2 stat_c3, --顾问
                            ...
                            decode(trunc(p.c81), trunc(sysdate-1), decode(p.c3, 240020, p.c1, ''), '') stat_c5, --入库中
       ...
       select seqB_Stat_HC.nextval, 260003, 130001, ttt.*
         from (select stat_c3, count(distinct stat_c4), count(distinct stat_c5), count(distinct stat_c6)
               ...
               from (select p.c2 stat_c3, --顾问
                            ...
                            decode(to_char(p.c81,'yyyymm'), to_char(sysdate,'yyyymm'), decode(p.c3, 240020, p.c1, ''), '') stat_c5, --入库中
       ...
       select seqB_Stat_HC.nextval, 260004, 130001, ttt.*
         from (select stat_c3, count(distinct stat_c4), count(distinct stat_c5), count(distinct stat_c6)
               ...
               from (select p.c2 stat_c3, --顾问
                            ...
                            decode(to_char(p.c81,'yyyymm'), to_char(sysdate-30,'yyyymm'), decode(p.c3, 240020, p.c1, ''), '') stat_c5, --入库中
       ...
       select seqB_Stat_HC.nextval, 260009, 130001, ttt.*
         from (select stat_c3, count(distinct stat_c4), count(distinct stat_c5), count(distinct stat_c6)
               ...
               from (select p.c2 stat_c3, --顾问
                            ...
                            decode(p.c3, 240020, p.c1, '') stat_c5, --入库中
...
create or replace package body pcace.pkg_pcace_web is
  ...
  is
    plog            number;
    v_workyear      number;
    v_jobStr        varchar2(2000);
    v_educationStr  varchar2(2000);
    v_education     number;
    v_age           number;
    v_xingzuo       number;
    v_birth_mm      varchar2(2);
    v_birth_dd      varchar2(2);
    v_mmName        varchar2(200);
    counter         number;
  begin
    plog := 100;
    ...
    plog := 101;
    select count(dataid) into counter from B_Candidate_CVEducation where c1=a_cid and c2=130001;
    if counter > 0 then
        select F_Candidate_MyEducation(a_cid) into v_educationStr from dual;
        select max(c7) into v_education from B_Candidate_CVEducation where c1=a_cid and c2=130001;
    else
        v_educationStr := '';
    end if;

    plog := 102;
    select count(dataid) into counter from B_Candidate_CVWork where c1=a_cid and c2=130001 and c5 is not null;
    if counter > 0 then
        plog := 103;
        select sum(to_number(substr(decode(c4,'至今',to_char(sysdate,'yyyy'),c4),1,4)) - to_number(substr(NVL(c3,to_char(sysdate,'yyyy')),1,4)))
          into v_workyear from B_Candidate_CVWork where c1=a_cid and c2=130001;      
        select F_Candidate_MyWork(a_cid) into v_jobStr from dual;

        select count(dataid) into counter from B_Candidate_HCComment where c1=a_cid and c3=130001;
        --c36,c41,c38,c40,c42,c43,c44,c45 最近工作信息
        --c64 屏蔽当前公司
        plog := 104;
        update B_Candidate t set
               (c36,c41,c38,c40,c42,c43,c44,c45,c64)= (select distinct a.c20, NVL(b.c2,a.c5), a.c7, decode(a.c4,'至今',1,0), a.c9, a.c11, a.c12, a.c13, decode(counter,0,a.c5,t.c64) from (select m.*,rank() over(order by m.c3 desc) ccc from B_Candidate_CVWork m where c1=a_cid and c2=130001 and c5 is not null) a, A_Organ b where ccc=1 and a.c5=b.c2(+)||'['||b.c1(+)||']')
         where c1=a_cid;
        ...
        if counter > 0 then
            update B_Candidate t set
                   (c41,c38,c40,c42,c43,c44,c45,c64)= (select distinct NVL(b.c2,t.c41), t.c38, 0, t.c42, t.c43, t.c44, t.c45, t.c41 from A_Organ b where t.c41=b.c2||'['||b.c1||']')
             where c1=a_cid;
        end if;
    ...
    plog := 110;
    update B_Candidate t set c24=(select c10 from B_Candidate_HCComment where dataid=(select max(dataid) from B_Candidate_HCComment where c1=a_cid and c3=130001)) where c1=a_cid;
  ...
  exception when others then
    rollback;
    Message := Message||'简历更新失败plog='||plog||'! '||substr(sqlerrm,1,100);
    open ResultCursor for select Message sqlmsg from dual;
  end;
  ...
  begin
    ...
    --入库审核规则:R00001
    DECLARE CURSOR v_rs IS select b.c1,b.c2,b.c4,b.c5,b.c6,b.c7,b.c8 from IS_APPModule a, IS_APPModuleDetail b where a.dataid=b.parentDataid and a.c6=130001 and b.c9=130001 and a.c1='R00001' order by b.c10,b.dataid;
    BEGIN
      open v_rs;
      LOOP
        FETCH v_rs INTO v_title, v_type, v_tableName, v_content, v_condition, v_orderby, v_index;
        exit when v_rs%notfound;
        ...
        if counter=0 then
          ...
          if v_type = 111 then
            v_sql := 'select '||v_content||' from (select rownum rownum_, t.* from (select * from '||v_tableName||' where '||v_condition||' and c1='||a_cid||' '||v_orderby||') t ) ttt where rownum_= 1';
          else
            v_sql := 'select '||v_content||' from '||v_tableName||' where '||v_condition||' and c1='||a_cid;
          end if;
    ...
    select count(a.dataid) into counter from B_Candidate_CVWork a, A_Organ b where a.c1=a_cid and a.c2=130001 and a.c5=b.c2||'['||b.c1||']';
    if counter = 0 then
        v_result := v_result || '<br>【工作经历】:<br>所在公司未在ACE公司库';
    end if;
  ...
  begin
    ...
    --顾问面评审核规则:R00002
    DECLARE CURSOR v_rs IS select b.c1,b.c2,b.c4,b.c5,b.c6,b.c7,b.c8 from IS_APPModule a, IS_APPModuleDetail b where a.dataid=b.parentDataid and a.c6=130001 and b.c9=130001 and a.c1='R00002' order by b.c10,b.dataid;
    BEGIN
      open v_rs;
      LOOP
        FETCH v_rs INTO v_title, v_type, v_tableName, v_content, v_condition,v_orderby, v_index;
        ...
        if counter=0 then
          ...
          elsif v_type = 113 then --顾问面评前 3 条必填 ，请看sql最后面的 rownum_ <= 3
            v_sql := 'select '||v_content||' from (select rownum rownum_, t.* from (select * from '||v_tableName||' where '||v_condition||' and c1='||a_cid||' '||v_orderby||') t ) ttt where rownum_ <= 1'; -- 2017-04-23[变更为 rownum_ <= 1] by wxl 【martin要求】
