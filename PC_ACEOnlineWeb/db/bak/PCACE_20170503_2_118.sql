...
-- Created by Administrator on 2017/5/3, 21:46:42 --
----------------------------------------------------

set define off
spool PCACE_20170503_2_118.log
  ...
  begin
    ...
    LOOP
      ...
      if length(v_tmp)>0 then
        Result := Result || '_SP1;' || v_tmp;
      end if;
    END LOOP;
    close v_rs;
  end;
  if length(Result)>0 then Result := substr(Result, length('_SP1;')+1); end if;
  return(Result);
...
begin
 ...
 ------------------- select count(a.dataid) into counter from A_Organ a where 
...
prompt
prompt Creating function F_ISVIEWED
prompt ============================
prompt
create or replace function pcace.F_isViewed(
  cid varchar2,
  hrAuid varchar2
) return number is
  counter number;
begin
  select count(dataid) into counter from B_T_Candidate_ViewLog where c1=cid and c2=hrAuid;
  if counter > 0 then
    return 1;
  end if;
  return(0);
...
create or replace package body pcace.pkg_pcace_web is
  ...
  begin
    ...
    if counter > 0 then
        ...
        select count(dataid) into counter from B_Candidate_CVEducation where c1=a_cid and c2=130001 and c7<>133010;
        if counter > 0 then
          select max(c7) into v_education from B_Candidate_CVEducation where c1=a_cid and c2=130001 and c7<>133010;
        else
    ...
    if counter > 0 then
        ...
        update B_Candidate t set
               (c36,c41,c38,c40,c42,c43,c44,c45,c64)= (select distinct a.c20, NVL(b.c2,a.c5), a.c7, decode(a.c4,'至今',1,0), a.c9, NVL(a.c11,t.c43), NVL(a.c12,t.c44), NVL(a.c13,t.c45), decode(counter,0,a.c5,t.c64) from (select m.*,rank() over(order by m.c3 desc) ccc from B_Candidate_CVWork m where c1=a_cid and c2=130001 and c5 is not null) a, A_Organ b where ccc=1 and a.c5=b.c2(+)||'['||b.c1(+)||']')
         where c1=a_cid;
