...
<%@ include file="/common/IncludeTop_s.jsp" %>
    ...
    <html:hidden name="B_DataModuleBean" property="ao.dataid" />
    <html:hidden name="B_DataModuleBean" property="ao.moduleID" />
...
%>
    ...
    <!--
      function go(cmd){
        location.href = "B_DataModule.y?cmd="+cmd;
      }      
      function edit(obj,id){
        var f = document.MainForm;
        f("ao.dataid").value = id;
        f.action = "B_DataModule.y?cmd=editPage";
        f.submit();
      } 
      function del(obj,id,name){
        confirmAction('确定要删除[' + name + ']吗？', function(){
            ajax('B_DataModule.y?cmd=del', {"ao.dataid":id}, function(html){
                setMessage(html);
                page_redirect();
            });
         });
      ...
      function page_redirect(page,pageKey){
        ...
        var f = document.MainForm;
        f.action = "B_DataModule.y?cmd=getRowsHead&editFlag=1&outFlag=1&h="+h;
